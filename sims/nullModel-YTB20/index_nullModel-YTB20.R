...
write.csv(para_grid, paste(gsub("/out/", "", outdir),"/para_grid.csv", sep=""), row.names=F)
...
### Initialise output for list of first and last RES distribution in each simulation (i.e. two lists) of length (sims)
### each.
RES = list()
...
for(sim in 1:sims) {
  ...
  ### Initialise custom land placement (this is independent of budget other than it will affect yield, and thus
  ### budgets). Type = "equal" is the default and distributes non-public land evenly among users. Type = "oneRich" takes
  ### a given mean proportion (hi_frac) and allocates this to one single user. Note there is some error around this
  ### (beta-distribution) with upper and lower bounds optionally set by up_frac and lo_frac.
  ...
  ### Initialise user budgets as function of yield 
  u_buds = set_budgets(prv = sim_old,                      # To extract remaining budget from previous
                       nxt = sim_old,                      # Current year sim results
  ...
  sim_old$AGENTS[2:(gmse_paras[["stakeholders"]]+1),17] = u_buds
  
  ### Initialise manager budget (with method man_bud_type) 
  sim_old$AGENTS[1,17] = set_man_budget(u_buds, 
  ...
  for(year in 1:years) {
    ...
    if(class(sim_new)=="try-error") {
      ...
      res_sim[[year]] = sim_new$RESOURCES[,5:6]
...
save(RES, file = sprintf("%sRES_%s.Rdata", outpath, outidx))

# Once done, set "done" flag in para_grid to 1 instead of -1:
para_grid[vals_idx,"done"] = 1
write.csv(para_grid, paste(gsub("/out/", "", outdir),"/para_grid.csv", sep=""), row.names=F)
