...
# 
# ### Is it just me or does it look as if gmse_apply() is consistently lower? Would need to "up" reps to check.
# 
# ### However, the following does the same as above, but increases agent_view and times_observe in an attempt to really 
# ###  reduce observation uncertainly - i.e. population trajectories should be even closer between gmse and gmse_apply()?
# 
# ### NOTE THIS OVERRIDES THE EARLIER RUNS!
# 
# ### GMSE first
# out_gmse = matrix(NA, nrow = reps, ncol = nyears)
# for(z in 1:reps) {
#   
#   gmse_z = gmse(time_max = 10, agent_view = 20, times_observe = 2, plotting = F)
#   out_gmse[z, ] = unlist(lapply(gmse_z$resource, nrow))
# }
# 
# out_gmseapply = matrix(NA, nrow = reps, ncol = nyears)
# for(z in 1:reps) {
#   print(sprintf("Rep %d", z))
#   z_old = gmse_apply(get_res = "Full", agent_view = 20, times_observe = 2)
#   out_gmseapply[z,1] = z_old$basic_output$resource_results
#   
#   for(i in 2:nyears) {
#     z_new = gmse_apply(get_res = "Full", old_list = z_old)
...
# ylo = 0.9*min(c(min(out_gmse),min(out_gmseapply)))
# yhi = 1.2*max(c(max(out_gmse),max(out_gmseapply)))
# plot(out_gmse[1,], type = "n", ylim = c(ylo, yhi), main = "agent_view = 20, times_observe = 2")
# legend(x = 4, y = yhi, legend = c("gmse()","gmse_apply()"), col = c("red","blue"), fill = c("red","blue"))
# apply(out_gmse, 1, function(x) lines(x, col = "red"))
# apply(out_gmseapply, 1, function(x) lines(x, col = "blue"))
# 
